...
public class MomagicBase {
    public var aps : Payload?
    public class func modelsFromDictionaryArray(array:NSArray) -> [MomagicBase]
    {
        var models:[MomagicBase] = []
        for item in array
        {
            models.append(MomagicBase(dictionary: item as! NSDictionary)!)
        }
        return models
    }
    required public init?(dictionary: NSDictionary) {
